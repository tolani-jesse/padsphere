...
export default function PresetEditorModal({ preset, onClose, onEditSounds, onRenameComplete, onDeleteComplete }: Props) {
  ...
  return (
    <div className="presets-overlay" style={{ alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div className="glass-panel" style={{ padding: '2rem', borderRadius: '12px', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '1.5rem', background: 'rgba(15, 19, 34, 0.95)' }} onClick={e => e.stopPropagation()}>
        ...
        {isDeleting ? (
          ...
          <button className="btn" style={{ textAlign: 'left', color: '#f87171', borderColor: 'rgba(248, 113, 113, 0.2)', display: 'flex', alignItems: 'center', gap: '0.6rem' }} onClick={() => setIsDeleting(true)}>
